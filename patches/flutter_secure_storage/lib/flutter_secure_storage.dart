...
class FlutterSecureStorage {
  ...
  Map<String, String> _selectOptions(
    ...
    if (kIsWeb) {
      ...
      // 鸿蒙 HarmonyOS NEXT:无官方 flutter_secure_storage 适配。本仓库自写
      // SecureStoragePlugin(原生 ArkTS,channel 'plugins.it_nomads.com/
      // flutter_secure_storage',AES-GCM 加密)。其协议与安卓实现一致,故鸿蒙
      // 分支复用 AndroidOptions(params 透传给原生,options 内容由原生忽略)。
      return aOptions?.params ?? this.aOptions.params;
    } else {
  ...
  /// 是否运行在鸿蒙(HarmonyOS)。
  ///
  /// 优先用 `Platform.operatingSystem == 'ohos'`(dart:io 标准属性,鸿蒙 Flutter
  /// 引擎 C++ 层注入,release AOT 稳定);兜底用 dynamic isOhos getter(debug 可用)。
  static bool get _isOhos {
    if (Platform.operatingSystem == 'ohos') return true;
    try {
      // ignore: avoid_dynamic_calls
      return (Platform as dynamic).isOhos as bool? ?? false;
    } catch (_) {
      return false;
    }
  }
