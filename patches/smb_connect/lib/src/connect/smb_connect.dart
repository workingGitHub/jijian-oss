...
abstract class SmbConnect {
  ...
  static String getUncPath(String path) {
    ...
    if (path.startsWith("\\")) {
      // 原实现 `afterToken("\\").afterToken("\\")` 对 share 根(如 /share,
      // 转换后 \share)会因第二个 afterToken 找不到 \ 而返回空,
      // 拼出畸形 UNC `\\`(实为 \\host\share\\),导致部分 SMB 服务器
      // (如群晖)在 listFiles 枚举时报 NT_STATUS_INVALID_NAME。
      // 修正:share 根(无子路径)返回 `\`(空相对路径),使 UNC 为合法的
      // `\\host\share\`;有子路径时保持原行为 `\\sub`。
      final afterShare = path.afterToken("\\").afterToken("\\");
      return afterShare.isEmpty ? "\\" : "\\$afterShare";
    } else {
